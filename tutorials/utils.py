...
# ---------------------------------------------------------------------------
# Resolve the datasets directory. Priority (first match wins):
#   1. BRAINIAK_DATA_PATH environment variable
#   2. DATA_PATH defined in config.py at the repo root (copy config.example.py)
#   3. Fallback: <repo>/brainiak_datasets (the legacy local layout)
# See README.md for how to point this at the Google Drive shared folder.
# ---------------------------------------------------------------------------
_repo_root = os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..'))

def _resolve_data_path():
    env_path = os.environ.get('BRAINIAK_DATA_PATH')
    if env_path:
    ...
    try:
        ...
        if getattr(_cfg, 'DATA_PATH', None):
            return os.path.expanduser(_cfg.DATA_PATH)
    except ImportError:
        pass
    return os.path.join(_repo_root, 'brainiak_datasets')

data_path = _resolve_data_path()

# Results path: Where the results and intermediate analyses of the tutorials are stored.
# Override with the BRAINIAK_RESULTS_PATH env var if you want outputs elsewhere.
results_path = os.path.expanduser(
    os.environ.get('BRAINIAK_RESULTS_PATH', os.path.join(_repo_root, 'brainiak_results'))
)
